...
static uint32_t tag[4];
static uint32_t iv[3];
...
int main(void) {
    for (unsigned bs = MIN_BLOCK_SIZE; bs <= MAX_BLOCK_SIZE; bs += 1) {
        size_t sz = 1 << bs;
        perfcounter_t tm_enc;
        perfcounter_t tm_dec;

        wipe_buffer_content(sz);

        perfcounter_config(COUNT_CYCLES, true);
        ime_aead_enc_mram(&key[0], NULL, sz, &buffer[0], &buffer[0], &tag[0], &iv[0]);
        tm_enc = perfcounter_get();

        perfcounter_config(COUNT_CYCLES, true);
        assert(ime_aead_dec_mram(&key[0], &iv[0], &tag[0], sz, &buffer[0], &buffer[0]));
