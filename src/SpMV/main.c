...
int main() {
    ...
    uint32_t numRowsPerTasklet = 0;
    if (numRows > 0) {
        numRowsPerTasklet = ROUND_UP_TO_MULTIPLE_OF_2((numRows - 1) / NR_TASKLETS + 1);
    }
    ...
    if(taskletNumRows > 0) {
        ...
        // Extract parameters
        const uint32_t rowPtrsOffset = params_w->dpuRowPtrsOffset;
        const uint32_t rowPtrs_m     = params_w->dpuRowPtrs_m;
        const uint32_t nonzeros_m    = params_w->dpuNonzeros_m;
        const uint32_t inVector_m    = params_w->dpuInVector_m;
        const uint32_t outVector_m   = params_w->dpuOutVector_m;
