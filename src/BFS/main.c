...
__dma_aligned struct DPUParams params_w;

static inline void wait_for_host_and_fetch(void) {
    if (me() == 0) {
        g_phase = 1;
        __ime_wait_for_host();  // fault/yield until host releases mux
        g_phase = 0;
        ...
        if (g_ctrl.cmd != CMD_EXIT)
            mram_read((__mram_ptr void const*)ARG_OFFSET,
              &params_w,
              ROUND_UP_TO_MULTIPLE_OF_8(sizeof(params_w)));
    }
    mybarrier_wait();
}
...
int main() {
    ...
    mutex_id_t mid = MUTEX_GET(nextFrontierMutex);

    while (1) {
        // Wait for host to provide cmd/args and wake us
        wait_for_host_and_fetch();
        ...
        if (numNodes > 0) {
            ...
            for (uint32_t node = taskletNodesStart; node < taskletNodesStart + taskletNumNodes; ++node) {
                ...
                for (uint32_t i = ptr; i < nxt; ++i) {
                    ...
                    if (!isSet(vis, nb % 64)) {
                        mutex_lock(mid);
                        uint64_t nft = load8B(nextFrontier_m, nbTile, cache_w);
                        setBit(nft, nb % 64);
                        store8B(nft, nextFrontier_m, nbTile, cache_w);
                        mutex_unlock(mid);
                    }
                }
            }
